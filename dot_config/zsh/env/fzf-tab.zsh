...
#
# Inside nvim's own :terminal this briefly needed a different key
# (right-arrow) too, on the theory that vim-tmux-navigator's global
# ctrl-hjkl mapping would claim ctrl-l before the terminal job saw it —
# but that mapping doesn't reach into terminal-insert mode, so ctrl-j/k
# already passed through fine, and so does ctrl-l once the popup is a
# properly-entered floating window (fixed alongside the sizing/position
# work). Back to one binding everywhere.
